...
export async function GET(req: NextRequest) {
  ...
  try {
    const snapshot = await marketingRpc("dashboard_snapshot", { p_token:token, p_days:days })
...
export async function POST(req: NextRequest) {
  ...
  if (action === "login") {
    const result = await marketingRpc<{ok:boolean;token?:string;expires_at?:string}>("dashboard_login", { p_password:String(data.password || "") })
  ...
  if (action === "logout") {
    await marketingRpc("dashboard_logout", { p_token:token }).catch(()=>{})
    const response = NextResponse.json({ ok:true })
    response.cookies.delete(COOKIE)
    return response
  }
  ...
  if (action === "change_password") {
    ...
    const result = await marketingRpc<{ok:boolean}>("dashboard_change_password", { p_token:token, p_new_password:next })
