...
type ContactMessage = {
  id: string; name: string; email: string; phone: string; company: string; serviceType: string; budget: string;
  message: string; locale: MarketingLocale; date: string; time: string; type: string; createdAt: string; city: string;
  source: string; marketingConsent: boolean
}

type CapturedLead = {
  ok:boolean; lead_id:string; lead_secret:string; unsubscribe_token:string; locale:MarketingLocale; should_schedule:boolean
}
...
function formatLeadHtml(msg: ContactMessage) {
  ...
  const rows = [["Tipo", msg.type], ["Nome", msg.name], ["Email", msg.email], ["Telefone", msg.phone], ["Empresa", msg.company], ["Serviço", msg.serviceType], ["Orçamento", msg.budget], ["Data da call", msg.date], ["Horário da call", msg.time], ["Idioma", msg.locale], ["Cidade", msg.city], ["Origem", msg.source], ["Marketing opt-in", msg.marketingConsent ? "Sim" : "Não"], ["Criado em", msg.createdAt]]
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name || !email || !message || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return NextResponse.json({ error: "Missing or invalid fields" }, { status:400 })
    ...
    const locale: MarketingLocale = country.toUpperCase() === "BR" || /^pt\b/i.test(acceptLanguage) ? "pt-BR" : safeMarketingLocale(String(data.locale || ""))
    const requestedSessionId = String(data.sessionId || "").slice(0,120)
    ...
    let sessionId: string | null = requestedSessionId.length >= 10 ? requestedSessionId : null
    ...
    try {
      if (sessionId) {
        try {
          await marketingRpc("track_visitor_event", {
            ...
            p_metadata:{ serviceType:String(data.serviceType || ""), budget:String(data.budget || ""), marketingConsent, stage:"pre_capture" },
          }, dataToken)
        } catch (trackingError) {
          console.warn("Contact analytics session unavailable", trackingError)
          sessionId = null
        }
      }
      ...
      captured = await marketingRpc<CapturedLead>("capture_marketing_lead", {
        ...
        p_email: email, p_name: name, p_phone: String(data.phone || "").slice(0,80) || null,
        p_company: String(data.company || "").slice(0,200) || null, p_locale: locale,
        p_country: country.slice(0,8) || null, p_region: region.slice(0,120) || null, p_city: city.slice(0,160) || null,
        p_source: source, p_first_path: String(data.path || "/contact").slice(0,500), p_consent: marketingConsent,
      }, dataToken)

      await marketingRpc("mark_lead_conversion", { p_lead_id:captured.lead_id, p_lead_secret:captured.lead_secret, p_kind:"form_submit" }, dataToken)

      if (marketingConsent && captured.should_schedule) {
        const lead = captured
        after(async () => {
          try {
            await scheduleNurtureSequence({ leadId:lead.lead_id, leadSecret:lead.lead_secret, email, name, locale:lead.locale || locale, unsubscribeToken:lead.unsubscribe_token }, dataToken)
          } catch (automationError) {
            console.error("Contact nurture scheduling warning", automationError)
          }
        })
      }
    } catch (crmError) {
    ...
    const msg: ContactMessage = {
      id:captured ? `lead_${captured.lead_id}` : `contact_${Date.now()}`, name, email,
    ...
    if (resendApiKey && toEmail) {
      try {
        ...
        const { error } = await resend.emails.send({ from:fromEmail, to:[toEmail], replyTo:msg.email, subject, html:formatLeadHtml(msg), text:`Novo lead pelo site\n\nNome: ${msg.name}\nEmail: ${msg.email}\nTelefone: ${msg.phone}\nEmpresa: ${msg.company}\nServiço: ${msg.serviceType}\nOrçamento: ${msg.budget}\nCidade: ${msg.city}\nOrigem: ${msg.source}\nMarketing opt-in: ${msg.marketingConsent ? "Sim" : "Não"}\n\nMensagem:\n${msg.message}` })
    ...
    } else warnings.push("E-mail administrativo não configurado neste ambiente.")

    return NextResponse.json({ success:true, id:captured?.lead_id || msg.id, scheduled:captured?.should_schedule ? 30 : 0, warning:warnings.length ? warnings.join(" ") : undefined })
  } catch (error) {
