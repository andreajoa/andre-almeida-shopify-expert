...
const projects: Record<LocaleKey, Project[]> = {
  "pt-BR": [
    { title: "NOVA AI Studio", category: "IA · SAAS · PRODUTO", description: "Uma plataforma de criação com IA transformada em produto digital com identidade, clareza e experiência de uso.", image: "/portfolio-real/nova-card.webp", href: "https://www.novvideos.online/", index: "01", featured: true },
    { title: "CAA Neuro", category: "APP · ACESSIBILIDADE · IA", description: "Tecnologia assistiva com comunicação alternativa, voz, tradução e recursos terapêuticos em uma experiência acessível.", image: "/portfolio-real/caa-neuro-card.webp", href: "https://www.caaneuro.online/", index: "02" },
    { title: "AMB Boutique", category: "E-COMMERCE · MODA", description: "E-commerce de moda feminina com direção editorial, experiência internacional, merchandising e jornada de compra pensada para conversão.", image: "/brand/amb-boutique-live.png", href: "https://www.ambboutique.online/", index: "03" },
    { title: "Brinqueteando", category: "E-COMMERCE · EDUCAÇÃO", description: "Loja de brinquedos terapêuticos com jornada digital pensada para confiança, clareza e conversão.", image: "/portfolio-real/brinqueteando-card.webp", href: "https://www.brinqueteando.online/", index: "04" },
  ],
  en: [
    { title: "NOVA AI Studio", category: "AI · SAAS · PRODUCT", description: "An AI creation platform shaped into a digital product with clear identity, positioning and user experience.", image: "/portfolio-real/nova-card.webp", href: "https://www.novvideos.online/", index: "01", featured: true },
    { title: "CAA Neuro", category: "APP · ACCESSIBILITY · AI", description: "Assistive technology combining alternative communication, voice, translation and therapeutic resources.", image: "/portfolio-real/caa-neuro-card.webp", href: "https://www.caaneuro.online/", index: "02" },
    { title: "AMB Boutique", category: "ECOMMERCE · FASHION", description: "A women’s fashion ecommerce experience combining editorial direction, international merchandising and a conversion-focused customer journey.", image: "/brand/amb-boutique-live.png", href: "https://www.ambboutique.online/", index: "03" },
    { title: "Brinqueteando", category: "ECOMMERCE · EDUCATION", description: "A therapeutic toy store with a digital journey built around trust, clarity and conversion.", image: "/portfolio-real/brinqueteando-card.webp", href: "https://www.brinqueteando.online/", index: "04" },
  ],
}
      ...
      <section id="selected-work" className="border-t border-[#d4cec2] py-24 md:py-32 lg:py-40">
        <div className="mx-auto max-w-[1600px] px-5 sm:px-8 lg:px-10 xl:px-14">
          ...
          <div className="mt-16 grid gap-5 lg:mt-24 lg:grid-cols-12">
            {selectedProjects.map((project, index) => <AnimatedSection key={project.title} delay={index * 0.04} className={project.featured ? "lg:col-span-7" : index === 1 ? "lg:col-span-5" : "lg:col-span-6"}><a href={project.href} target="_blank" rel="noopener noreferrer" onClick={() => Analytics.ctaClick(`premium_project_${project.title}`)} className="group block h-full border-t border-[#cfc8bc] pt-4"><div className={`relative overflow-hidden bg-[#d8d1c5] ${project.featured ? "aspect-[1.16/1]" : "aspect-[1.1/1]"}`}><div className="absolute inset-x-4 top-1/2 aspect-[45/31] -translate-y-1/2 overflow-hidden bg-white shadow-[0_24px_60px_rgba(42,38,31,0.18)] sm:inset-x-7"><Image src={project.image} alt={`Projeto ${project.title}`} fill quality={80} className="object-contain transition duration-700 ease-out group-hover:scale-[1.012]" sizes={project.featured ? "(max-width: 1024px) 92vw, 54vw" : "(max-width: 1024px) 92vw, 38vw"}/></div></div><div className="grid grid-cols-[1fr_auto] gap-5 py-5 sm:py-6"><div><p className="text-[9px] font-semibold tracking-[0.16em] text-[#807b71]">{project.category}</p><h3 className="mt-3 font-editorial text-3xl leading-none tracking-[-0.03em] sm:text-4xl">{project.title}</h3><p className="mt-3 max-w-xl text-sm leading-6 text-[#6c675e]">{project.description}</p></div><div className="text-right"><span className="text-[10px] font-semibold tracking-[0.15em] text-[#807b71]">{project.index}</span><span className="mt-8 hidden items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.14em] sm:flex">{c.viewProject}<ArrowUpRight className="h-3.5 w-3.5" /></span></div></div></a></AnimatedSection>)}
          </div>
